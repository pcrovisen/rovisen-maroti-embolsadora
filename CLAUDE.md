...
2. Machines are `Machine<TState>`: `internal class Foo : Machine<Foo.States>` with a nested `public enum States`. The enum must be `public`/`internal` (a private nested type cannot be the base's type argument) and the machine's `Name` — `class name + identifier` — is what the HMI and `WebHMI/wwwroot/transitions.json` key on, so `PalletLabel` instantiated with identifier `"1"` must stay `PalletLabel1`. Mark states that mean a failure with `[FaultState]`; the diagnostics page draws them differently.
3. **The wire contract lives in `Contracts/` (`Wenco.Contracts`)** and is referenced by both projects: DTOs (`SystemStatus`, `Pallet`, `Packager`, `Car`, `Connections`, `ErrorMessages`, `DeletePallet`, `HmiConfig`), the `Signals` coil enum with `SignalIndex`, and `PalletEntryState`. Change it once; the compiler enforces both sides. Keep it dependency-free — it must stay referenceable from the HMI without dragging the server's packages in. Coil numbers are not array indices: use `SignalIndex.Plc/Pc`. The HMI protocol is length-prefixed (4-byte LE int) on both sides — a change to the framing, or to the contract, means deploying server and HMIs together. The Web HMI mirrors the same enums in JavaScript, generated (see the build/run notes above).
